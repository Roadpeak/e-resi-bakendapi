"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SavedPropertiesController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const current_user_decorator_js_1 = require("../common/decorators/current-user.decorator.js");
const pagination_dto_js_1 = require("../common/dto/pagination.dto.js");
const saved_properties_service_js_1 = require("./saved-properties.service.js");
let SavedPropertiesController = class SavedPropertiesController {
    service;
    constructor(service) {
        this.service = service;
    }
    findMine(user, pagination) {
        return this.service.findMine(user.id, pagination);
    }
    save(user, slug) {
        return this.service.save(user.id, slug);
    }
    unsave(user, slug) {
        return this.service.unsave(user.id, slug);
    }
    isSaved(user, slug) {
        return this.service.isSaved(user.id, slug);
    }
};
exports.SavedPropertiesController = SavedPropertiesController;
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'List saved properties' }),
    __param(0, (0, current_user_decorator_js_1.CurrentUser)()),
    __param(1, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, pagination_dto_js_1.PaginationDto]),
    __metadata("design:returntype", void 0)
], SavedPropertiesController.prototype, "findMine", null);
__decorate([
    (0, common_1.Post)(':slug'),
    (0, swagger_1.ApiOperation)({ summary: 'Save a property' }),
    __param(0, (0, current_user_decorator_js_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('slug')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], SavedPropertiesController.prototype, "save", null);
__decorate([
    (0, common_1.Delete)(':slug'),
    (0, swagger_1.ApiOperation)({ summary: 'Remove a property from saved list' }),
    __param(0, (0, current_user_decorator_js_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('slug')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], SavedPropertiesController.prototype, "unsave", null);
__decorate([
    (0, common_1.Get)(':slug/status'),
    (0, swagger_1.ApiOperation)({ summary: 'Check if a property is saved' }),
    __param(0, (0, current_user_decorator_js_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('slug')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], SavedPropertiesController.prototype, "isSaved", null);
exports.SavedPropertiesController = SavedPropertiesController = __decorate([
    (0, swagger_1.ApiTags)('Saved Properties'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)('saved-properties'),
    __metadata("design:paramtypes", [saved_properties_service_js_1.SavedPropertiesService])
], SavedPropertiesController);
//# sourceMappingURL=saved-properties.controller.js.map